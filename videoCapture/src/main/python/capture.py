...
def detectFaces(image):
    ...
    faces = faceDetector.detectMultiScale(equalized, minSize=(35, 35), minNeighbors=4)
    croppedFaces = []
    x = y = x_w = y_h = 0
...
def trainFace(g_capture):
    ...
    writerNormal = cv2.VideoWriter("train" + name + ".avi", fourcc, 30, framesize, True)
    if writerNormal is None or not writerNormal.isOpened():
        print "Unable to open video file writer"
        exit(1)
    ...
    if writerNormal.isOpened():
        print "release opened writer"
        writerNormal.release()
...
def recognizeFace(image, position, name):
    ...
    if name is not None:
        ...
        # saveSnapshot(image, "old_face_"+name+".png")
...
def recognizedFaceMain(g_capture):
    ...
    framesize = (width, height)
    writerNormal = cv2.VideoWriter("testnormal.avi", fourcc, 30, framesize, True)
    if writerNormal is None or not writerNormal.isOpened():
        print "Unable to open video file writer"
        exit(1)
    ...
    while True:
        try:
            ...
            flag, image = g_capture.read()
            if image is not None:
                name, position = recognizeFace(image, position, name)
                writerNormal.write(image)
...
def main():
    global var, g_capture, height, width, fourcc
    print cv2.__file__
    loadLabels()
